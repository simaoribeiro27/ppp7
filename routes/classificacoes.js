...
/* GET ALL     /classificacoes */
router.get('/classificacoes/:idMomentos/momento', function (req, res, next) {
     var idMomentos = req.body.idMomentos;
  db.classificacoes.find({ idMomentos: req.params.idMomentos }, function(err,  classificacoes) {
        if (err) { res.send(err); }
        console.log('Todas as classificacoes: ' + JSON.stringify(classificacoes));
        res.json(classificacoes);
    });
